...
### Added
- Cursor 2.5+ marketplace manifests (`.cursor-plugin/`) curating the open-source-eligible packs, so the pack line installs from inside Cursor (RR-B-29).

### Changed
- Landing page (`docs/index.html`): refreshed catalogue to 46 packs grouped by publisher, added "Why it pays off" and "Using a pack" sections, self-hosted brand fonts, and removed every em dash from the copy (RR-B-24, RR-B-28).
- Reconciled all version strings to 1.16.2 from one source.

### Fixed
- Enabled the GitHub Pages landing site (`main:/docs`) and set the repository homepage URL (RR-B-20, RR-B-21).

## [1.16.1]: 2026-06-24

### Fixed
- Corrected four legacy Topic-Index mis-routes flagged by the routing-truth gate (`pack_eval`): `dodaf` (Traceability OV to SV), `mil-std-882` (User representative), `nasa-npr-7123` (Bidirectional traceability), and `nasa-system-safety` (Graded approach). Each term now routes only to chapters that actually ground it; no chapter content changed. All 46 content packs pass routing truth.
...
- **`nist-stat-handbook`** (8 ch): NIST/SEMATECH e-Handbook of Statistical Methods (NIST
  ...
  growth). Applied frequentist statistics for measurement and quality. It does NOT reproduce the
...
- **`nasa-rm-standard`** (6 ch): NASA Reliability and Maintainability Standard
  ...
  An objectives-and-strategies standard; it defers step-by-step procedures to the referenced
...
- **`se-standards-signpost`** is a *signpost*, not a knowledge pack: maps the whole SE
...
- **`omg-signpost`** is a *signpost*, not a knowledge pack: zero OMG content, just spec names,
...
- **`omg-sysml`** pack was pulled from the catalogue. The OMG Specification License public grant
