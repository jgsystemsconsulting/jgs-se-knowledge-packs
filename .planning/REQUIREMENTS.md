...
- [ ] **VET-20-01**: Retry FUT-04 Army CBA Guide (ASAFM PDF). Record dated evidence. Build pack only with in-source redistribution grant; otherwise keep FUT-04 deferred with fresh evidence (not a silent tick)
- [ ] **VET-20-02**: Licence spot-check AAF Product Support Manager Guidebook + AAF Software pathway guidebooks before any use. Quote in-source grant or keep Excluded-pending / "NOT yet vetted — do not use"
- [ ] **VET-20-03**: Optional ROSAP Rev E reachability check vs current `faa-std-025` Rev F mirror — document only; no forced rebuild
